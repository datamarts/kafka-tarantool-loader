...
build:
	$(CMD) /bin/bash -c "cd /sbroad/; git pull; cargo build --release; cd /memstore; cartridge build; cp /kafka/kafka/tntkafka.so /memstore/.rocks/lib/tarantool/kafka/tntkafka.so; cp /sbroad/target/release/libsbroad.so /memstore/.rocks/lib/tarantool/sbroad.so"
